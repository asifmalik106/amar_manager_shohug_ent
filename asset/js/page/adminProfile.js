...
function editCustomer()
{
	...
	var editSCLimit = $('#editSCLimit').val();
	 $.post(baseURL+'admin/customer/editProfile/',
	    {
		 			editSCID: editSCID,
...
function loadProfile(){
	var scID = $('#scID').val();
	$.post(baseURL+'admin/customer/getProfile/'+scID+'/',
	    {
		submit: "true"
	},
	    function(data, status){
...
function loadAllInvoice(){
	...
	$.post(baseURL+'admin/customer/getInvoices/'+scID+'/',
	    {
		submit: "true"
	},
	    function(data, status){
...
function loadAllTransactions(){
	...
	$.post(baseURL+'admin/customer/getTransactions/'+scID+'/',
	   {
		submit: "true"
	},
	    function(data, status){
...
function addCashToInvoice()
{
  ...
  $("#addCashToInvoice").prop("disabled",true);
	...
	console.log(scID+" "+invID+" "+payment+" "+fromBalance);
	 $.post(baseURL+'admin/invoice/payment',
	    {
	        scID: scID,
	        invID: invID,
		 			payment: payment,
...
function payCashToInvoice()
{
	...
	$.post(baseURL+'admin/invoice/purchasePayment',
	{
		scID: scID,
		invID: invID,
		cashID: cashID,
...
function addCashAdvance()
{
	...
	var scID = $('#scID').val();
	if( x == y ){
			$.post(baseURL+'admin/customer/addAdvance',
	    {
	        scID: scID,
	        advAmount: x,
		 			note: advNote,
...
function addCashAdvancePayment()
{
	...
	if( payment1 == payment2 ){
		$.post(baseURL+'admin/supplier/addAdvance',
		{
			scID: scID,
			cashID: cashID[0],
			payment: payment1,
...
function withdrawBalanceSubmit()
{
	...
	var scID = $('#scID').val();
	if( x == y ){
			$.post(baseURL+'admin/customer/withdrawBalance',
	    {
	        scID: scID,
	        wAmount: x,
		 			note: wNote,
...
function withdrawBalanceSubmitSupplier()
{
	...
	var scID = $('#scID').val();
	if( x == y ){
			$.post(baseURL+'admin/supplier/withdrawBalance',
	    {
	        scID: scID,
	        wAmount: x,
		 			note: wNote,
...
{
	var scID    = $('#scID').val();
	 $.post(baseURL+'admin/customer/getBalance',
	    {
	        scID: scID,
	    },
	    function(data, status){
